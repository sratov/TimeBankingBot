...
# 2. Для тестирования через ngrok, используйте переменные окружения:
#    - API_URL для бэкенда
#    - FRONTEND_URL для фронтенда
...
class TestTimeBankingApp(unittest.TestCase):
    ...
    def authenticate_user(self, user_id=None, username=None):
        ...
        try:
            # Отправляем POST с init_data в параметрах запроса
            ...
            if response.status_code == 200:
                ...
                try:
                    ...
                    logger.error(f"Debug authentication failed: {str(debug_err)}")
